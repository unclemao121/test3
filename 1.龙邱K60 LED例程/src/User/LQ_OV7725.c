...
//    DMA_PORTx2BUFF_Init (DMA_CH4, (void *)&PTD_BYTE0_IN,(void*)Image_Data, PTD13, DMA_BYTE1, (IMAGEW * 2 > 511 ? 511:IMAGEW * 2), DMA_rising); //一次DMA传输最大512个字节 
    DMA_PORTx2BUFF_Init (DMA_CH4, (void *)&PTD_BYTE0_IN,(void*)Image_Data, PTA19, DMA_BYTE1, (IMAGEW ), DMA_rising); //一次DMA传输最大512个字节 
}
...
__ramfunc void Get_Use_Image(void)
{
  ...
  for(i = 0; i  < IMAGEH; i+=4)  //240行，每4行采集一行，
  {
    for(j = 0;j < IMAGEW; j+=4)  //320 / 4  = 80，
    {        
...
{ 
  int i = 0, j = 0; 
  UART_Put_Char(UART4,0x01); //发送帧头标志 WindowsFormsApplication1.exe
  UART_Put_Char(UART4,0xfe); //发送帧头标志 WindowsFormsApplication1.exe
  for(i=0;i < IMAGEH; i++) 
  { 
    for(j=0;j <IMAGEW ;j++) 
    { 
  ...
  UART_Put_Char(UART4,0xfe); //发送帧尾标志 
  UART_Put_Char(UART4,0x01); //发送帧尾标志 
} 

void UARTSendPicture(uint8_t tmImage[IMAGEH][IMAGEW]) 
{ 
  int i = 0, j = 0; 
  UART_Put_Char(UART4,0xFF); //发送帧头标志 DEMOK上位机  
  for(i=0;i < IMAGEH; i++) 
  { 
    for(j=0;j <IMAGEW;j++) 
    { 
